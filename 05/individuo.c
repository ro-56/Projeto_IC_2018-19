                         ...
                         int PERIODOS_ANO)
{
    ...
    individuo novo_ind;
    int *f_obj = (int *)calloc(ESPECIES, sizeof(int));

    /*  Inicializa o vetor demanda atendida de cada individuo  */
    int *demanda_atendida = (int *)calloc(N_OBJ, sizeof(int));
    ...
    while (p_walker)
    {
        ...
        while (p_walker->free_time < PERIODOS)
        {
    ...
    f_obj[0] = calculateFObj (ESPECIES,
                              demanda,
                              lucrativity,
                              demanda_atendida);
    
    f_obj[1] = (int)((1.0 / ((float) f_obj[0] + 100)) * 10000);
